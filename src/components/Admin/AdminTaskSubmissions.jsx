...
import { collection, onSnapshot, updateDoc, doc, getDocs, query, where, deleteDoc } from "firebase/firestore";
...
export default function AdminTaskSubmissions() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Вы действительно хотите удалить это задание? Это действие необратимо.")) {
      setLoading(true);
      await deleteDoc(doc(db, "task_submissions", id));
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#191A1D] p-8 text-white">
      <div className="max-w-6xl mx-auto">
        <button
          ...
          className="mb-6 bg-gray-700 hover:bg-gray-600 text-white font-semibold px-4 py-2 rounded transition"
        >
          ← Назад к пользователям
        </button>
        <h2 className="text-3xl font-bold mb-8">Проверка индивидуальных заданий</h2>
          ...
          <table className="min-w-full divide-y divide-gray-700">
            <thead>
              <tr>
                ...
                <th className="px-6 py-3 text-left font-semibold">Задание</th>
              ...
              {(submissions && submissions.length === 0) && (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-400">
                    Нет новых индивидуальных заданий для проверки
                  </td>
                </tr>
              )}
              {submissions &&
                ...
                <tr key={s.id} className="hover:bg-[#282B30] transition">
                  ...
                  <td className="px-6 py-4">{s.assignmentId}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className="bg-gray-800/30 text-white px-2 py-1 rounded-full text-xs font-medium">
                      {s.status || "—"}
                    </span>
                  </td>
